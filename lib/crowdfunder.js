...
class CrowdFunder {
    ...
    getProjects(startIndex, count, callback) {
        ...
        this.dataConnection.query("SELECT id, title, subtitle FROM Projects ORDER BY id LIMIT ? OFFSET ?", [count < 0 ? 200 : count, startIndex], (err, rows) => {
    ...
    getProject(id, callback) {
        ...
        "SELECT Projects.title, Projects.subtitle, Projects.description, Projects.target, Projects.creationDate, COUNT(Backers.user_id), SUM(Backers.amount) " +
            ...
            "WHERE Backers.project_id = ?;", [id, id, id, id], (err, rows) => {
            if (err) {
    ...
    getRewards(id, callback) {
        ...
        this.dataConnection.query("SELECT Rewards.id, Rewards.amount, Rewards.description " +
            "FROM Rewards " +
            "WHERE Rewards.project_id = ?", [id], (err, rows) => {
            if (err) {
    ...
    getImage(id, callback) {
        this.dataConnection.query("SELECT imageData FROM Projects WHERE id=?", [id], (err, rows) => {
            if (err) {
    ...
    getUser(id, callback) {
        this.dataConnection.query("SELECT username, email, location FROM Users WHERE id=?", [id], (err, rows) => {
            if (err) {
                callback({
                    httpCode: 500
                });
    ...
    getLoginStatus(token, callback) {
        const id = token.id;
        this.dataConnection.query("SELECT logoutTime FROM Users WHERE id=?", [id], (err, rows) => {
            if (err) {
                callback({
                    httpCode: 500
                });
