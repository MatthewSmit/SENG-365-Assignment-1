...
function getProjects(startIndex, count, callback) {
    ...
    dataConnection.query("SELECT id, title, subtitle FROM Projects ORDER BY id LIMIT ? OFFSET ?", [count < 0 ? 200 : count, startIndex], (err, rows) => {
...
function getImage(id, callback) {
    dataConnection.query("SELECT imageData FROM Projects WHERE id=?", [id], (err, rows) => {
        if (err) {
